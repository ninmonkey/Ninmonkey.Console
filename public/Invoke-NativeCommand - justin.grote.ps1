...
$dotnetManifest = Invoke-NativeCommand {
    dotnet new tool-manifest -o $ProjectPath
} -Catch {
    if ($PSItem -match 'Creating this template will make changes to existing files') {
        #This is OK, it means it already exists
    } else { throw $PSItem }
}